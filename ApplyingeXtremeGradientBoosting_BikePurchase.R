#Reading data
data <- read.csv("~/LearningR/AW_data.csv", header = TRUE)
str(data)
View(data)
...
# XGboost requires numeric matrix so we will keep the Bike buyer variable as integer.
...
testm <- sparse.model.matrix(BikeBuyer~. -1, data = test)
head(testm)
test_label <- test[,"BikeBuyer"]
...
# We store it in best model (bst_model)
...
min(e$test_mlogloss)
e[e$test_mlogloss == 0.421452,]


# Now we will look at more features in the model to optimize it.Low values of eta ensures model is robust to overfitting.
bst_model <- xgb.train(params = xgb_params, data= train_matrix, nrounds = 89, watchlist = watchlist, eta = 0.05)
...
# clearly we see Number of children at home is the most important variable.
...
bst_model <- xgb.train(params = xgb_params, data= train_matrix, nrounds = 89, watchlist = watchlist, eta = 0.05, max.depth =6, gamma = 0)


# More modifications. Lower values of sub-sample helps to reduce overfitting.
bst_model <- xgb.train(params = xgb_params, data= train_matrix, nrounds = 1000, watchlist = watchlist, eta = 0.01, max.depth =3, gamma = 0, subsample = 1, colsample_bytree = 1, missing = NA, seed = 333)
e <- data.frame(bst_model$evaluation_log)
plot(e$iter,e$train_mlogloss, col = 'blue')
lines(e$iter,e$test_mlogloss, col = 'red')

min(e$test_mlogloss)
e[e$test_mlogloss == 0.426814,]
